...
class ConnectionHandler:
    """
    WebSocket 连接处理器基类，负责单个客户端连接的生命周期管理。
    """
    ...
    async def handle_connection(self) -> None:
        ...
        try:
            ...
            if not await self._authenticate(auth_data):
                ...
                return
            # 2. 注册会话
            ConnectionHandler.sessions[self.user_id] = self.websocket
            ...
            # 4. 断开连接
            if self.user_id in ConnectionHandler.sessions:
                del ConnectionHandler.sessions[self.user_id]
